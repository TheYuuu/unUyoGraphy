...
export default class DotMatrixHistogram extends ChartBase {
  ...
  private yAxis_g: Selection<SVGGElement, unknown, null, undefined> | null =
    null;
  private xAxis_g: Selection<SVGGElement, unknown, null, undefined> | null =
    null;
  ...
  private draw(): void {
    ...
    const xAxis = axisBottom(xPosAxis).ticks(3).tickSize(padding);
    ...
    const yAxis = axisLeft(yPosAxis).ticks(3).tickSize(padding);
  ...
  private updateDots(
    ...
    dots
      ...
      .attr(
        "transform",
        `translate(${padding + leftPadding}, ${-(padding + bottomPadding)})`
      )
      .attr("r", (d) => dotWidth / 2)
      .transition()
      .duration(duration)
  ...
  private updateYAxis(axis: Axis<NumberValue>): void {
    ...
    yAxis_g
      .attr(
        "transform",
        `translate(${padding + leftPadding}, ${
          containerHeight - maxHieght - bottomPadding - padding
        })`
      )
      .transition()
      .duration(duration)
    ...
    xAxis_g
      .attr(
        ...
        `translate(${leftPadding}, ${
          containerHeight - bottomPadding - padding
        })`
      )
      .transition()
      .duration(duration)
